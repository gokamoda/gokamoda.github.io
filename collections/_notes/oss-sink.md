...
Now, due to the presence of $b^{S}_h$, the following holds:

$$
\begin{align}
&1 = \frac{\exp(b^{S}_h)}{\exp(b^{S}_h)+\sum_{j'} \exp(s_{i, j', h})} + \sum_j \alpha_{i, j, h}\\
&\Leftrightarrow \sum_j \alpha_{i, j, h} = 1 - \frac{\exp(b^{S}_h)}{\exp(b^{S}_h)+\sum_{j'} \exp(s_{i, j', h})}
\end{align}
$$

Let the attention weight assigned to the sink be expressed as follows:
